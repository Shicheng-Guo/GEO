...
phen2<-sapply(strsplit(as.character(phen$characteristics_ch1.3),": "),function(x) unlist(x)[2])  # age
phen3<-sapply(strsplit(as.character(phen$characteristics_ch1.4),": "),function(x) unlist(x)[2])  # gender
...
data1=na.omit(data)
PCAPlot(t(data1),phen1,output="GSE42861_status.pca.pdf",multifigure=T)
PCAPlot(t(data1),phen3,output="GSE42861_gender.pca.pdf",multifigure=T)
PCAPlot(t(data1),phen2,output="GSE42861_age.pca.pdf",multifigure=T)
